#!/usr/bin/env bash
# FMP API helper for EdenFinTech Scanner (Stable API)
...
case "$cmd" in
    ...
    profile)
        ...
        curl -s "$BASE/profile?symbol=$ticker&apikey=$FMP_API_KEY"
        ;;
    ...
    income)
        ...
        curl -s "$BASE/income-statement?symbol=$ticker&period=annual&limit=10&apikey=$FMP_API_KEY"
        ;;
    ...
    balance)
        ...
        curl -s "$BASE/balance-sheet-statement?symbol=$ticker&period=annual&limit=10&apikey=$FMP_API_KEY"
        ;;
    ...
    cashflow)
        ...
        curl -s "$BASE/cash-flow-statement?symbol=$ticker&period=annual&limit=10&apikey=$FMP_API_KEY"
        ;;
    ...
    ratios)
        ...
        curl -s "$BASE/ratios?symbol=$ticker&period=annual&limit=10&apikey=$FMP_API_KEY"
        ;;
    ...
    metrics)
        ...
        curl -s "$BASE/key-metrics?symbol=$ticker&apikey=$FMP_API_KEY"
        ;;
    ...
    price-history)
        ...
        curl -s "$BASE/historical-price-eod/light?symbol=$ticker&apikey=$FMP_API_KEY"
        ;;
    ...
    ev)
        ...
        curl -s "$BASE/enterprise-values?symbol=$ticker&period=annual&limit=10&apikey=$FMP_API_KEY"
        ;;
    ...
    peers)
        ...
        curl -s "$BASE/stock-peers?symbol=$ticker&apikey=$FMP_API_KEY"
        ;;

    # SBC data from cash flow
    # Usage: fmp-api.sh sbc TICKER
    sbc)
        ticker="${1:?Usage: fmp-api.sh sbc TICKER}"
        curl -s "$BASE/cash-flow-statement?symbol=$ticker&period=annual&limit=5&apikey=$FMP_API_KEY" | \
            python3 -c "
import sys, json
...
"
    ...
    list-nyse)
        curl -s "$BASE/_company-screener_?exchange=NYSE&isActivelyTrading=true&limit=5000&apikey=$FMP_API_KEY"
        ;;

    help|*)
        echo "EdenFinTech FMP API Helper (Stable API)"
        echo ""
